...
test('season details reject invalid routes before contacting TMDB', async (t) => {
  let calls = 0;
  const upstream = async () => { calls++; return Response.json({}); };
  for (const path of ['/details/tv/0/season/1', '/details/tv/1/season/0', '/details/movie/1/season/1']) {
...
test('rejects invalid IDs/types and POST before contacting TMDB', async (t) => {
  ...
  for (const path of ['/details/person/0', '/details/movie/0', '/details/tv/nope', '/details/movie/99999999999999999', '/details/movie/1/credits']) {
    assert.equal((await request(t, path, upstream)).status, 400);
  }
...
test('person details return only normalized safe fields', async (t) => {
  const response = await request(t, '/details/person/287', async (url, options) => {
    ...
    return Response.json({
      ...
      birthday: '1963-12-18', known_for: [
        ...
        null, { title: 'A'.repeat(2) }, { title: 'Too many' },
