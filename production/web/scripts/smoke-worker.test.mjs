...
import { runSmoke, validateBase, validateOrigin } from './smoke-worker.mjs';
...
test('requires an HTTPS frontend origin', () => {
  assert.throws(() => validateOrigin('http://showtimetracker.show'), /HTTPS/);
  assert.equal(validateOrigin('https://showtimetracker.show'), 'https://showtimetracker.show');
});
...
test('checks the fixed public route table without printing response bodies', async () => {
  ...
  try {
    ...
    await runSmoke(validateBase(`http://127.0.0.1:${port}`), 'https://showtimetracker.show');
  } finally {
